...
namespace DS_GameLib
{
    public static class GameFunction
    {
        ...
        {
            // Combat method between 2 things
            // Health variables
            ...
            while ((pla_hp > 0) && (mob_hp > 0))
            {
                ...
                // Damage var
                int damage = 0;
                Random rand = new Random(); // Extra random because for some reason the numbers wont change in rounds

                // Player turn
                if (pla_hp > 0)
                {
                    //damage = GetDamage(pla_die, rand.Next(0,5));
                    damage = GetDamage(pla_die);
                    mob_hp -= damage;
                    Console.WriteLine($"Player does {damage} damage to Mob.");
                }

                // Mob turn
                if (mob_hp > 0)
                {
                    //damage = GetDamage(mob_die, rand.Next(0, 5));
                    damage = GetDamage(mob_die);
                    pla_hp -= damage;
                    Console.WriteLine($"Mob does {damage} damage to Player.");
                }
        ...
        public static int GetDamage(string die)
        {
            // Initialize variables
            int damage = 0;
            Random rand = new Random(); // Random variable

            // Get damage from die string
            string[] dmg_range = die.Split('D'); // Damage Modifier

            Console.WriteLine(dmg_range[0]);
            Console.WriteLine(dmg_range[1]);
            Console.ReadLine();

            //damage = rand.Next(1, int.Parse(dmg_range[1])) * int.Parse(dmg_range[0]);
            damage = rand.Next(1, 12) * 1;

            // Return damage
            return damage;
        }

        public static int GetDamage(string die, int seed)
        {
            // Initialize variables
            int damage = 0;
            Random rand = new Random(seed); // Random variable
